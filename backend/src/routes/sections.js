...
router.post('/', requireAuth, requireRole('admin', 'teacher'), async (req, res) => {
  try {
    ...
    await logAction(req.dbUser, 'section.create', {
      courseCode: section.courseCode, sectionNumber: section.sectionNumber, term: section.term, capacity: section.capacity,
    });
...
router.patch('/:id', requireAuth, requireRole('admin', 'teacher'), async (req, res) => {
  ...
  await logAction(req.dbUser, 'section.update', {
    courseCode: section.courseCode, sectionNumber: section.sectionNumber, term: section.term, changes: req.body,
  });
  res.json(section);
});
